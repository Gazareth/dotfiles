...
fn expression_list_with_binary_child_is_flattened() {
    // An expression_list whose sole child is a binary_expression should flatten to operands.
    let el_range = range(0, 0, 0, 11);
    let be_range = range(0, 0, 0, 11);
    let a_range  = range(0, 0, 0, 1);
    let b_range  = range(0, 6, 0, 11);

    // expression_list with a single child collapses under Rule 1 (no selection to make).
    // In practice it is reached via hint navigation (e.g. "v" from an assignment outline).
    let focus = NodeOutline { node_type: "expression_list".into(), range: el_range.clone(), child_count: 1 };
    let root  = NodeOutline::new("chunk",           el_range.clone());
    let ancestry = NodeAncestry::new_test(vec![focus], root, Language::Lua);

    // Snapshot 1: expression_list with one binary_expression child.
    ...
    // Snapshot 2: binary_expression — its two operands (consumed by flatten_binary_outline).
    snap("binary_expression")
        .child_ranged("identifier", "a", a_range.start_row, a_range.start_col, a_range.end_row, a_range.end_col)
        .child_ranged("identifier", "b", b_range.start_row, b_range.start_col, b_range.end_row, b_range.end_col)
        .queue();

    // Hint simulates navigating directly to "expression_list" (e.g. the "v" outline key).
    let result = FocusedNode::from_ancestry(ancestry, Some(("expression_list", 0, 0))).unwrap().unwrap();
    let outline = &result.outline;

    assert_eq!(outline.len(), 2, "should flatten through binary_expression to show the two operands");
    assert!(outline.iter().all(|i| i.node_type != "binary_expression"), "no binary_expression items should remain");
    assert_eq!(outline[0].node_type, "identifier");
    assert_eq!(outline[0].range.start_col, 0);
    assert_eq!(outline[1].node_type, "identifier");
    assert_eq!(outline[1].range.start_col, 6);

...
function M.wrap_item(item, session)
  if item.action_id == "jump_to_child" then
    ...
    item.action = function()
      require("configs.hydra.atlantis-deprecated.prepare.anchor_point.build.jump_child_picker").open(reopen_args, session.hydra_opts)
    end
    return
  end
  ...
  if item._action_common_overflow == true and type(item._overflow_action_names) == "table" then
    ...
    local reopen_args = type(item.reopen) == "table" and item.reopen or {}
    item.action = function()
      require("configs.hydra.atlantis-deprecated.prepare.anchor_point.build.action_overflow_picker").open(
        reopen_args,
        session.hydra_opts,
        names
      )

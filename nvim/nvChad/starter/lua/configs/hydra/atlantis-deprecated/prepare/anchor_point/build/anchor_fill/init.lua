local nav_column = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.build.anchor_fill.nav_column")
...
function M.fill(anchor_node_info, find_result, menu_opts)
  ...
  if not parsed_anchor then
    parsed_anchor = probe.parse(anchor_node_info, { config = treesitter_config.get() })
  end
  return {

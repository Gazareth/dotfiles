local parse_identifier = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.probe.node_kinds.identifier").parse_identifier
local parse_assignment = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.probe.node_kinds.assignment").parse_assignment
local parse_function = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.probe.node_kinds.function").parse_function
...
local parse_binary_expression = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.probe.node_kinds.binary_expression").parse_binary_expression
local parse_generic = require("configs.hydra.atlantis-deprecated.prepare.anchor_point.probe.node_kinds.generic").parse_generic
local kinds = require("configs.hydra.atlantis-deprecated.schema.constants")
...
-- opts.semantic: pre-resolved semantic (skips languages.resolve).
-- opts.config:   pre-resolved treesitter config (skips treesitter_config.get()).
-- Both are optional; probe falls back to internal resolution when absent.
function M.parse(node_info, opts)
	...
	elseif type(opts.config) == "table" then
		config = opts.config
		semantic = resolve_language_mapping(node_info, {
			safe_languages = config.safe_languages,
			languages = config.languages,
		})
	else

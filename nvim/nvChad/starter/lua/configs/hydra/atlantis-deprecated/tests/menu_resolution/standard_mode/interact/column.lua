...
local anchor_schema = require("configs.hydra.atlantis-deprecated.schema.actions.anchor")
...
return function()
  describe("Column", function()
    local function assert_inventory_and_labels(node_kind, anchor_ctx)
      ...
      -- jump_to_comment only appears when a preceding comment exists; fixtures here have none.
      local conditional = { jump_to_comment = true }
      for name, on in pairs(enabled) do
        if on == true and not conditional[name] then

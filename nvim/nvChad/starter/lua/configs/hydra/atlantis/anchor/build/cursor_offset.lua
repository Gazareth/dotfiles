...
local function resolve_menu_depth_one(anchor_node_info, cursor_node_info)
  ...
  cur = cursor_node_info.node
  while cur do
    if cur:type() == "return_statement" then
      ...
      local fn_node =
        node_common.find_ancestor_of_types(cursor_node_info.node, function_constants.function_like_types, {
          include_self = true,
        })
      if fn_node then

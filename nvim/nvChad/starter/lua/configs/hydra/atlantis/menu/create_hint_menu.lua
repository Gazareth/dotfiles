...
function M.create_hint_menu(layout)
  if layout.variant == "no_cursor" then
    return assembly.build_without_cursor(layout.navigate_spec)
  end
  if layout.variant == "invalid_action_menu" then
    return assembly.build_with_invalid_action_menu(layout.positioned_anchor_node_info, layout.navigate_spec)
  end

...
function M.build_without_cursor()
  return {
    title = "Treewalker",
    sections = section_order.build_default(),
  }
end

-- Build menu payload when modify section failed to produce a table
function M.build_with_invalid_modify(positioned_anchor_node_info)
  return {
    title = "Treewalker",
    sections = section_order.build_default(),
...
function M.build_with_abort_modify(modify_spec, positioned_anchor_node_info)
  return {
    title = "Treewalker",
    sections = section_order.with_modify(modify_spec),
    anchor_node_info = positioned_anchor_node_info,
  }
end

-- Build normal menu payload with titled modify section
function M.build_with_modify(modify_spec, positioned_anchor_node_info)
  local menu_title = modify_spec.title or "Treewalker"

  return {
    title = menu_title,
    sections = section_order.with_modify(build_titled_modify_spec(modify_spec)),
    anchor_node_info = positioned_anchor_node_info,
  }
end

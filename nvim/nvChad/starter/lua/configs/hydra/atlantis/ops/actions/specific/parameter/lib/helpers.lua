-- Resolve parameter context state and execute target jump/swap helpers
...
-- Execute a Treewalker command token safely
local function run_treewalker(command_token)
  local ok = pcall(vim.cmd, "Treewalker " .. command_token)
  if not ok then
    vim.notify("Treewalker command is unavailable: " .. tostring(command_token), vim.log.levels.WARN)
    return false
  end
...
-- Single parameter swap step using Treewalker sibling swapping
local function swap_parameter_step(direction)
  local command_token = direction == "next" and "SwapRight" or "SwapLeft"
  if not run_treewalker(command_token) then
    return false
  end

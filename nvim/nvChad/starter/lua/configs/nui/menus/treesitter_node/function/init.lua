local actions = require("configs.nui.menus.treesitter_node.function.lib.actions")
local navigation = require("configs.nui.menus.treesitter_node.function.lib.navigation")
local parsed_helpers = require("configs.nui.menus.treesitter_node.function.lib.parsed")
...
function M.build(node_info, parsed)
  ...
  local items = {
    ...
    {
      separator = true,
      label = "Parameters: " .. tostring(parameter_count),
    },
  }
  ...
  items[#items + 1] = {
    separator = true,
    label = "Nested Functions: " .. tostring(nested_count),
  }
  ...
  items[#items + 1] = {
    separator = true,
    label = "Assigns: " .. tostring(assignment_count),
  }
  ...
  items[#items + 1] = {
    separator = true,
    label = "Lines: " .. tostring(line_span) .. ", Called: " .. tostring(called_count),
  }
  items[#items + 1] = { separator = true }
